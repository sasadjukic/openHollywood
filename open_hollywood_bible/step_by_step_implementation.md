...
and creates plans from fully configured persisted presets. Remaining before
completion: implement the real full-story `BenchmarkCaseExecutor`, run the
formal corpus across all three profiles and the single-model baseline within an authorized budget, collect blind human reviews, and preserve the resulting campaign evidence. Evidence so far: `benchmarks/v0.1/corpus.json`,
`engine/open_hollywood_engine/evaluations/`, `scripts/evaluation_harness.py`, and `tests/evaluations/`.
