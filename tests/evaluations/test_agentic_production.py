    ...
    async def generate(self, request: ModelRequest) -> ModelResponse:
        ...
        if role == "scene_writer":
            ...
            content["requirement_coverage"] = {
                entry["id"]: _positive_requirement_coverage(entry, payload)
...
def _positive_requirement_coverage(
    entry: dict[str, object], payload: dict[str, object]
) -> dict[str, object]:
    del entry
    candidate = cast(dict[str, object], payload["candidate_draft"])
    content = cast(dict[str, object], candidate["content"])
    evidence_catalog = cast(list[dict[str, str]], content["evidence_catalog"])
    return {
        "status": "met",
        "coverage_assessment": "The candidate performs this exact obligation.",
        "evidence_refs": [evidence_catalog[0]["evidence_ref"]],
    }
...
def _v13_contradiction_inputs(
    ...
    catalog = payload["contradiction_claim_catalog"]
...
def _schema_test_continuity_context(*, recheck: bool = False) -> _ContinuityModelContext:
    return _ContinuityModelContext(
        ...
        canonical_source_catalog=(
            {
                ...
                "artifact_version_id": str(uuid4()),
                "source_path": "content.established_facts[0]",
                "claim": "Mara has not returned the brass key.",
                "claim_id": "story_bible_brass_key_established_facts",
                "categories": ["fact"],
                "related_ids": ["scene_1"],
            },
        ),
        ...
        requirement_satisfaction_modes={"required_element_1": "include"},
        requirement_enforcement={"required_element_1": "blocking"},
...
def _v17_catalog_test_execution(
    *,
    ...
    draft_prose: str = "Mara pockets the brass key.",
) -> _Execution:
    plan = {
        "id": "scene_1",
            ...
            },
            {
                ...
                "content": {
                    "established_facts": [
                        {
                            "id": "brass_key_fact",
                            "statement": "Mara has not returned the brass key.",
                            "character_ids": [],
                            "location_ids": [],
                        }
                    ],
...
class OneProductionRepairGateway(ProductionFixtureGateway):
    ...
    async def generate(self, request: ModelRequest) -> ModelResponse:
        ...
        if isinstance(continuity_contract, dict):
            self.continuity_contracts.append(continuity_contract)
        ...
        schema_repair = payload.get("schema_repair")
        if isinstance(schema_repair, dict):
            self.schema_repairs.append(schema_repair)
            ...
            content["findings"] = [
                {
                    ...
                    "basis_details": {
                        ...
                        "conflict_explanation": "The propositions conflict.",
...
class ContinuityRevisionFeedbackGateway(ProductionFixtureGateway):
    ...
    async def generate(self, request: ModelRequest) -> ModelResponse:
        ...
        if role == "continuity_supervisor":
            ...
            if revision_number > 0:
                ...
                prior_finding_id = cast(str, reports[0]["findings"][0]["id"])
    ...
    def _blocking_response(
        ...
        *,
        ...
        if is_recheck:
            assert prior_finding_id is not None
            ...
            content["prior_finding_rechecks"] = {
                prior_finding_id: {
                    ...
                    "repair_assessment": finding["repair_assessment"],
                    "revised_draft_evidence_refs": selected_evidence_refs,
                }
            }
            content["new_findings"] = []
...
class HybridStagnationEscalationGateway(ProductionFixtureGateway):
    ...
    def _blocking_response(
        ...
        if payload.get("output_schema_variant") == "recheck":
            ...
            content.pop("findings", None)
            previous = cast(dict[str, object], payload["previous_continuity_report"])
            previous_content = cast(dict[str, object], previous["content"])
            previous_findings = cast(list[dict[str, object]], previous_content["findings"])
            prior_finding_id = cast(str, previous_findings[0]["id"])
            content["prior_finding_rechecks"] = {
                prior_finding_id: {
                    "status": "still_blocking",
                    "repair_assessment": finding["repair_assessment"],
    ...
    finding = _materialize_continuity_evidence(
        {
            ...
            "basis": "contradiction",
            "category": "fact",
            "canonical_claim_ids": [context.canonical_claim_ids[0]],
    ...
    findings = _materialize_requirement_audit(
        {
            "requirement_coverage": {
                "required_element_1": {
                    ...
                    "evidence_refs": [],
                    "evidence_search_result": "no_related_passage",
                }
            },
        },
        context,
    ...
    findings = _materialize_requirement_audit(
        {
            "requirement_coverage": {
                "scene_plan_outcome": {
                    ...
                    "evidence_refs": [context.evidence_refs[0]],
                    "evidence_search_result": "closest_passages_selected",
                }
            }
        },
        context,
                ...
                "findings": [
                    {
                        ...
                        "category": "fact",
                        "basis": "contradiction",
                        "summary": "The key action conflicts with canon.",
...
def test_initial_continuity_evidence_is_bound_to_the_current_draft() -> None:
    ...
    claim_id = next(
        claim_id
        for claim_id, categories in context.canonical_claim_categories.items()
...
def test_v21_scene_plan_requirements_are_exclusive_to_keyed_coverage() -> None:
    ...
    assert [entry["id"] for entry in context.requirement_catalog] == ["required_element_1"]


def test_v23_non_world_certificate_derives_redundant_fields_in_application() -> None:
    execution = _v17_catalog_test_execution(scene_plan={"outcome": "Mara returns the brass key."})
    context = _continuity_model_context(execution)
    claim_id = context.canonical_claim_ids[0]
    source_ref = context.canonical_claim_source_refs[claim_id]
    model_finding: dict[str, object] = {
        "severity": "blocking",
        "summary": "Mara keeps the key instead of returning it.",
        ...
        "basis_details": {
            ...
            "conflict_disposition": "directly_incompatible",
            "conflict_explanation": "Keeping the key conflicts with the established state.",
            "repair_action": "replace",
        },
    }
    ...
    assert isinstance(finding, dict)
    assert finding["canonical_source_refs"] == [source_ref]
    assert finding["category"] == "fact"
    assert finding["draft_assertion"] == "Mara pockets the brass key."
    assert finding["conflict_kind"] == "fact"
    assert finding["related_scene_ids"] == []
    _validate_continuity_finding_contract([finding], execution)

    model_supplied_metadata = {
        **model_finding,
        "category": "timeline",
        "canonical_source_refs": ["invented_source"],
        "related_scene_ids": ["scene_99"],
    }
    rematerialized = _materialize_continuity_evidence(model_supplied_metadata, context)
    assert isinstance(rematerialized, dict)
    assert rematerialized["category"] == "fact"
    assert rematerialized["canonical_source_refs"] == [source_ref]
    assert rematerialized["related_scene_ids"] == []


def test_v21_no_scene_plan_field_is_a_selectable_contradiction_claim() -> None:
    execution = _v17_catalog_test_execution(
        scene_plan={
            "title": "Debating the Artifacts",
            "outcome": "Mara returns the brass key.",
        }
    )

    catalog = _continuity_model_context(execution).canonical_source_catalog
...
def test_v22_direct_non_world_conflict_remains_blocking() -> None:
    finding = {
        ...
        "conflict_explanation": "The draft and canonical key states conflict.",
    }
...
def test_v21_pursue_uses_shared_coverage_statuses_without_requiring_achievement() -> None:
        ...
        requirement_catalog=(
            {
                ...
                "category": "character",
                "text": "Mara tries to recover the key.",
                "satisfaction_mode": "pursue",
                "enforcement": "blocking",
            },
        ),
        requirement_kinds={"scene_plan_goal": "scene_plan_requirement"},
        requirement_categories={"scene_plan_goal": "character"},
        requirement_satisfaction_modes={"scene_plan_goal": "pursue"},
        requirement_enforcement={"scene_plan_goal": "blocking"},
    )
    ...
    coverage = schema["$defs"]["RequirementCoverageEntry"]["anyOf"]

    assert [branch["properties"]["status"]["const"] for branch in coverage] == [
        "met",
        "partial",
        "absent",
    ]
    assert "evidence_refs" in coverage[0]["required"]
    assert (
        _materialize_requirement_audit(
            {
                "requirement_coverage": {
                    "scene_plan_goal": {
                        "status": "met",
                        "coverage_assessment": "Mara actively searches for the key.",
...
@pytest.mark.parametrize(
    "source_field", ("entry_state", "time_context", "purpose", "goal", "conflict", "exit_state")
)
def test_v21_scalar_scene_plan_gaps_are_advisory(source_field: str) -> None:
    execution = _v17_catalog_test_execution(
        scene_plan={source_field: "Dusk settles over the long shadows."}
    )
    context = _continuity_model_context(execution)
    requirement_id = f"scene_plan_{source_field}"
    findings = _materialize_requirement_audit(
        {
            "requirement_coverage": {
                requirement_id: {
                    "status": "absent",
                    "summary": f"The {source_field} is not explicit.",
                    "coverage_assessment": "The draft leaves it implicit.",
                    "recommended_resolution": "Make the context explicit if useful.",
                    "evidence_refs": [],
                    "evidence_search_result": "no_related_passage",
                }
            }
    ...
    assert findings[0]["basis"] is None
    _validate_continuity_finding_contract(cast(list[object], findings), execution)


def test_v21_exact_dusk_evidence_satisfies_time_context() -> None:
    execution = _v17_catalog_test_execution(
        scene_plan={"time_context": "Dusk settles over the long shadows."},
        draft_prose="Dusk light stretches long shadows across the courtyard.",
    )
    context = _continuity_model_context(execution)
    ...
    findings = _materialize_requirement_audit(
        {
            "requirement_coverage": {
                "scene_plan_time_context": {
                    "status": "met",
                    "coverage_assessment": "The draft explicitly stages the scene at dusk.",
                    "evidence_refs": [context.evidence_refs[0]],
                }
            }
        },
...
def test_v21_partial_hard_requirement_is_auditable_advisory() -> None:
    context = _schema_test_continuity_context()
    findings = _materialize_requirement_audit(
        {
            "requirement_coverage": {
                "required_element_1": {
                    "status": "partial",
                    "summary": "Mara handles the key but does not return it.",
                    "coverage_assessment": "The closest passage shows the key in her hand.",
                    "recommended_resolution": "Complete the return action.",
                    "evidence_refs": [context.evidence_refs[0]],
                }
            }
        },
    ...
    assert findings[0]["severity"] == "warning"
    assert findings[0]["basis"] is None
    assert findings[0]["coverage_status"] == "partial"
    assert findings[0]["coverage_evidence"] == ["Mara pockets the brass key."]


def test_v21_recheck_discards_semantically_duplicate_new_finding() -> None:
    base = _schema_test_continuity_context(recheck=True)
    claim_id = base.canonical_claim_ids[0]
    source_ref = base.canonical_claim_source_refs[claim_id]
    prior_id = "continuity_issue_existing"
    context = replace(
        base,
        previous_continuity_report={
            "artifact_key": "continuity_scene_1",
    ...
    duplicate_new = {
        **retained,
        "id": "continuity_issue_duplicate",
        "recheck_disposition": "newly_exposed",
    }

    assert _consolidate_continuity_semantic_duplicates([retained, duplicate_new], context) == [
        retained
    ]
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (ModelDeployment.CLOUD, "schema_validation_failed", True),
    ),
)
def test_schema_repair_guidance_is_provider_neutral_for_structured_failures(
...
def test_v23_repair_names_exact_requirement_partition() -> None:
    context = _schema_test_continuity_context()
    guidance = _schema_repair_guidance(
        operation=_Operation.CONTINUITY,
        deployment=ModelDeployment.LOCAL,
...
def test_v23_repair_does_not_repeat_global_claim_maps() -> None:
    context = _schema_test_continuity_context()
    guidance = _schema_repair_guidance(
        operation=_Operation.CONTINUITY,
        deployment=ModelDeployment.LOCAL,
        previous_failure={
            ...
            "validation_issues": [
                {
                    "location": "findings.0.canonical_claim_ids",
                    "type": "non_world_claim_lineage_mismatch",
                    "message": "claim lineage does not match",
                }
            ],
        },
        continuity_schema_variant=_ContinuitySchemaVariant.INITIAL_CHECK,
    ...
    assert guidance is not None
    directive = cast(list[dict[str, object]], guidance["directives"])[0]
    assert set(directive) == {"location", "issue_type", "action"}
    assert "exactly one self-describing" in cast(str, directive["action"])
    assert "remove the finding" in cast(str, directive["action"])


def test_v23_safe_expected_and_received_values_flow_into_cloud_repair() -> None:
    issues = _structured_failure_issues(
        _StructuredOutputContractError(
            "findings.0.conflict_disposition",
            "the direct-conflict disposition is invalid",
            issue_type="non_world_conflict_certificate_invalid",
            expected_value="directly_incompatible",
            received_value="inconsistent",
        )
    )

    assert issues[0]["expected_value"] == "directly_incompatible"
    assert issues[0]["received_value"] == "inconsistent"
    guidance = _schema_repair_guidance(
        operation=_Operation.CONTINUITY,
    ...
    assert guidance is not None
    directive = cast(list[dict[str, object]], guidance["directives"])[0]
    assert directive["expected_value"] == "directly_incompatible"
    assert directive["received_value"] == "inconsistent"
    assert "conflict_disposition=directly_incompatible" in cast(str, directive["action"])


def test_v23_story_length_guidance_uses_accepted_words_and_remaining_scenes() -> None:
    base = _v17_catalog_test_execution(draft_prose=" ".join(["accepted"] * 100))
    current = {
        "artifact_kind": ArtifactKind.SCENE_DRAFT.value,
        "artifact_key": "scene_2_draft",
        "artifact_version_id": str(uuid4()),
        "content": {
            "scene_id": "scene_2",
            "scene_number": 2,
            "revision_number": 0,
            "prose": " ".join(["current"] * 700),
        },
    }
    execution = replace(
        base,
        inputs=(*base.inputs, current),
        constraints={
    ...
    assert _story_length_guidance(execution) == {
        "policy": "story_wide_advisory",
        "story_target_words": {"minimum": 2_500, "maximum": 5_000},
        "accepted_scene_words_so_far": 100,
        "remaining_story_target_words": {"minimum": 2_400, "maximum": 4_900},
        "remaining_scenes_including_current": 3,
        "current_scene_soft_allocation_words": {"minimum": 800, "maximum": 1_634},
        "enforcement": "advisory_only; never revise, reject, or block solely for scene word count",
        "current_candidate_words": 700,
    }


def test_v23_story_wide_word_count_only_critique_cannot_force_revision() -> None:
    execution = replace(
        ...
        "issues": [
            {
                ...
                "severity": "blocking",
                "description": "This scene is under the minimum word count target.",
                "evidence": ["The draft contains about 900 words."],
                "recommendation": "Expand the scene to at least 2,500 words.",
            }
        ],
        "verdict": "revise",
    }

    normalized = _normalize_story_length_critique(critique, execution)
...
def test_v23_structural_critique_remains_revision_driving() -> None:
    execution = replace(
        _v17_catalog_test_execution(),
        constraints={
            "target_word_count": {"minimum": 2_500, "maximum": 5_000},
        ...
        "issues": [
            {
                ...
                "severity": "blocking",
                "description": (
                    "The causality is broken because Mara leaves before the motivating event; "
                    "this remains a defect even though the scene is under its word-count target."
                ),
                "evidence": ["Mara left. Then the alarm sounded."],
                "recommendation": "Move the alarm before Mara's exit.",
            }
        ],
        "verdict": "revise",
    }
...
def test_initial_continuity_schema_omits_every_recheck_only_field() -> None:
    ...
    coverage_entry = definitions["RequirementCoverageEntry"]
    partial = coverage_entry["anyOf"][1]
    missing = coverage_entry["anyOf"][2]
    ...
    assert "category" not in world_properties
    ...
    assert "category_details" not in contradiction["properties"]
...
def test_continuity_recheck_schema_exposes_recheck_analysis_fields() -> None:
    ...
    contradiction = basis_branches[0]
    world_contradiction = basis_branches[1]
    missing = definitions["RequirementCoverageEntry"]["anyOf"][2]
    world = world_contradiction["properties"]["category_details"]
    ...
    assert "evidence_refs" in missing["required"]
    assert {
        "world_rule_ids",
        "violation_kind",
        "rule_conflict_assessment",
...
def test_v18_recheck_schema_requires_each_exact_prior_finding_key() -> None:
    ...
    recheck_entry = schema["$defs"]["PriorFindingRecheckEntry"]
    assert all("finding" not in branch["properties"] for branch in recheck_entry["anyOf"])
    assert recheck_entry["anyOf"][0]["required"] == [
        "status",
        "resolution_assessment",
        "revised_draft_evidence_refs",
    ]
    assert recheck_entry["anyOf"][1]["required"] == [
        "status",
        "repair_assessment",
        "revised_draft_evidence_refs",
    ]
...
async def test_same_continuity_finding_inherits_resolution_across_recheck(
    ...
    for request in initial_requests:
        ...
        assert "contradiction_claim_catalog" in payload
        assert "world_rule_catalog" in payload
        ...
        source_catalog = payload["contradiction_claim_catalog"]
        ...
        assert all("reference_id" not in entry for entry in source_catalog)
        assert all("artifact_version_id" not in entry for entry in source_catalog)
    ...
    for request in recheck_requests:
        ...
        assert payload["output_schema_delivery"] == "enforced_by_local_gateway"
        assert "output_schema" not in payload
        assert payload["continuity_contract"]["coverage_statuses"] == [
            "met",
            "partial",
            "absent",
        ]
...
async def test_repeated_missing_continuity_resolution_fails_after_bounded_retry(
    ...
    assert "required_correction" not in repair_context
    ...
    assert len(gateway.schema_repairs) == 1
    schema_repair = gateway.schema_repairs[0]
    assert schema_repair["focus_locations"] == ["findings.0"]
    assert "recheck_disposition" in str(schema_repair["operation_rules"])
    with session_factory() as session:
...
async def test_repeated_story_bible_invariant_failure_is_actionable_and_bounded(
    ...
    assert len(gateway.schema_repairs) == 1
    story_bible_rules = str(gateway.schema_repairs[0]["operation_rules"])
